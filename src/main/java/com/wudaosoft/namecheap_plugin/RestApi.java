...
public class RestApi {
    ...
    private static final String LOGIN_PAGE_URL = "https://www.namecheap.com/myaccount/login/?ReturnUrl=%2fdashboard";
    ...
    public RestApi(String username, String password) {
        ...
        this.request = Request
                .custom().setHostConfig(HostConfigBuilder.create("https://ap.www.namecheap.com")
                        .setConnectTimeout(TIME_OUT).setSocketTimeout(TIME_OUT).build())
                .setRequestInterceptor(new HeadersInterceptor()).build();
    }
    ...
    public Map<String, String> loginParams() throws Exception {
        ...
        JSONObject obj = request.get(SESSION_HANDLER_AJAX_URL + new Date().getTime()).withAjax().withContext(session)
                .json();
        ...
        doc.select("form")
                .append(String.format(
                        "<input type=\"hidden\" name=\"sessionEncryptValue\" id=\"sessionEncryptValue\" value=\"%s\">",
                        obj.getString("SessionKey")));

        Elements els = doc.select("form[name=\"aspnetForm\"] input");

        return els.stream().collect(
                Collectors.toMap((e) -> e.attr("name"), Element::val, (key1, key2) -> key2, LinkedHashMap::new));
    }
    ...
    public boolean login() throws Exception {
        ...
        String local = request.doRequest(request.post(LOGIN_PAGE_URL, params).withContext(session), new ResponseHandler<String>() {
    ...
    public JSONArray getDomainList() throws Exception {
        ...
        JSONObject obj = request.post(GET_DOMAIN_LIST_AJAX_URL).withStringBody(payLoad).withAjax().withContext(session)
                .json();
    ...
    public boolean removeDomainDnsRecord(int hostId, int recordType, String domainName) throws Exception {
        ...
        JSONObject obj = request.post(REMOVE_DOMAIN_DNS_RECORD_AJAX_URL).withStringBody(payLoad).withAjax()
                .withContext(session).json();

        return "true".equals(obj.get("Result"));
    }

    public int addOrUpdateHostRecord(int hostId, String host, String value, int recordType, String domainName)
            throws Exception {
        ...
        JSONObject obj = request.post(ADD_OR_UPDATE_HOST_RECORD_AJAX_URL).withStringBody(payLoad).withAjax()
                .withContext(session).json();
